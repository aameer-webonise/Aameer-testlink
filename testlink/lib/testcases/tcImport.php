...
if ($args->do_upload)
{
	...
	{ 
		...
		if($gui->file_check['status_ok'] && $pimport_fn)
		{
			...
			importXLSFile($db,$dest,$args->container_id,$args->tproject_id,$args->userID,$opt);
...
$gui->importTypes=array('XML' => 'XML','XLS' => 'XLS'); 
...
function importXLSFile(&$db,$fileName,$parentID,$tproject_id,$userID,$useRecursion,$importIntoProject = 0)
{
  $testcases=readxlsx($fileName);
  
	$size=count($testcases);
	$conn=connectToDB();
	//$size=count($testcases);
	for($i=0;$i<$size;$i++){
		$sql = 'INSERT INTO nodes_hierarchy (name, parent_id, node_type_id,node_order) values('."'".$testcases[$i]['name']."'".','.$parentID.','.'3'.','.'100'.')';
		if ($conn->query($sql) === TRUE) {
			//echo 'test case '.$testcases[$i]['name'].'<br />';
			//print_r("New record created successfully");
		} else {
    		print_r("Error: " . $sql . "----" . $conn->error);
		}
		$sql='SELECT MAX(id) as maxid from nodes_hierarchy';
		...
		if($result->num_rows > 0)
		{
			...
			print_r('Please try after some time');
		}
		$sql='INSERT INTO nodes_hierarchy (name,parent_id,node_type_id,node_order) values('."''".','.$id.','.'4'.','.'1'.')';
		if ($conn->query($sql) === TRUE) {
			//print_r("New record created successfully in nodea_hierarchy");
		} else {
    		print_r("Error: " . $sql . "----" . $conn->error);
		}
		...
		$sql='SELECT MAX(tc_external_id) as maxid from tcversions';
		$result=$conn->query($sql);
		if($result->num_rows > 0)
		{
			$row = $result->fetch_assoc();
			$externalid=$row['maxid']+1;
			//print_r('<br />'.'max id is '.$id);
		}
		else{
			$externalid=1;
		}

		$sql='INSERT INTO tcversions (id,tc_external_id,version,layout,status,summary,preconditions,importance,creation_ts,modification_ts,active,is_open,execution_type) values ('.
		$id.','.$externalid.','.'1'.','.'1'.','.'1'.','."'".$testcases[$i]['summary']."','".$testcases[$i]['precondition']."',".'1'.','.'1'.','.'1'.','.'1'.','.'1'.','."'".$testcases[$i]['execution_type']."'".')';
		if ($conn->query($sql) === TRUE) {
			//print_r("New record created successfully tcversion");
		} else {
    		print_r("Error: " . $sql . "----" . $conn->error);
		}
		...
		$stepsSize=count($testcases[$i]['steps']);
		$stepNumber=1;
		for($k=0;$k<$stepsSize;$k++){
			
			$sql='INSERT INTO nodes_hierarchy (name,parent_id,node_type_id,node_order) values('."''".','.$stepParentID.','.'9'.','.'0'.')';
			if ($conn->query($sql) === TRUE) {
				//print_r("New record created successfully in nodea_hierarchy");
			} else {
    			print_r("Error: " . $sql . "----" . $conn->error);
			...
			$sql='INSERT INTO tcsteps (id,step_number,actions,expected_results,active,execution_type) values('.$id.','.$stepNumber.",'".$testcases[$i]['steps'][$k]."','".$testcases[$i]['expresults'][$k]."',".
			'1'.','.'1'.')';
			if ($conn->query($sql) === TRUE) {
				//print_r("New record created successfully in nodea_hierarchy");
				//echo 'inserted '.$testcases[$i]['steps'][$k].'<br />';
			} else {
    			print_r("Error: " . $sql . "----" . $conn->error);
			}
			$stepNumber++;
...
$xls_row_qty=$objXLS->setActiveSheetIndex(0)->getHighestRow();
$testcases=array();
...
for($idx = FIRST_DATA_ROW,$i=0; $idx <= $xls_row_qty; $idx++,$i++)
	{
		$testcases[$i]=array();
		$testcases[$i]['name']= htmlspecialchars($objXLS->getSheet(0)->getCell('A'.$idx)->getValue());
		
		$summary = convert_special_char($objXLS->getSheet(0)->getCell('B'.$idx)->getValue());  
		$summary = nl2p(htmlspecialchars($summary));
		$testcases[$i]['summary']=$summary;
		
		$precondition = convert_special_char($objXLS->getSheet(0)->getCell('C'.$idx)->getValue());  
		$precondition = nl2p(htmlspecialchars($precondition));
		...
		$j=0;
		do{
			$steps = convert_special_char($objXLS->getSheet(0)->getCell('E'.$idx)->getValue());
	    	$steps = nl2p(htmlspecialchars($steps));
			$testcases[$i]['steps'][$j]=$steps;
			
			$expresults = convert_special_char($objXLS->getSheet(0)->getCell('F'.$idx)->getValue());
			$expresults = nl2p(htmlspecialchars($expresults));
			$testcases[$i]['expresults'][$j]=$expresults;
			$idx++;
			$j++;
			$len=strlen($objXLS->getSheet(0)->getCell('A'.$idx)->getValue());
		}while(($len==0) && ($idx <= $xls_row_qty));
		$idx--;
	}
...
function create_xml_tcspec_from_xls($xls_filename,$xml_filename) 
{
	...
	{                       
		...
		fwrite($xmlFileHandle,"<summary><![CDATA[" . $summary . "]]></summary>\n");
		 ...
		 $precondition = convert_special_char($xls_rows[$idx][IDX_COL_PRECONDITIONS]);  
		$precondition = nl2p(htmlspecialchars($precondition));
		fwrite($xmlFileHandle,"<preconditions><![CDATA[" . $summary . "]]></preconditions>\n");
...
function getTestCaseSetFromSimpleXMLObj($xmlTCs)
{
	...
	for($idx = 0; $idx < $loops2do; $idx++)
	{
        ...
        $tc = $dummy[0]; 
       // print_r('bellow dummy ');
		if ($tc)
		{
			// Test Case Steps
			$steps = getStepsFromSimpleXMLObj($xmlTCs[$idx]->steps->step);
			//$steps = getStepsFromSimpleXMLObj($xmlTCs);
			$tc['steps'] = $steps;
...
function getStepsFromSimpleXMLObj($simpleXMLItems)
{
	...
	if( !is_null($items) )
	{
		var_dump('inside items if '.$items);
		$loop2do = count($items);
		for($idx=0; $idx < $loop2do; $idx++)
		{
			...
			if( isset($items[$idx]['expectedresults']) )
			{
				...
				print_r('result '.$items[$idx]['expected_results']);
