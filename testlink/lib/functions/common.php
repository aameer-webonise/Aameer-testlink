...
//Aameer Ausekar
function downloadXLSFile($testsuiteData,$testcase,$tcversions,$steps,$fileName){
	$objPHPExcel = new PHPExcel();
			// Set properties
			$objPHPExcel->getProperties()->setCreator("Webonise lab")
							->setTitle("Excel Format For TestLink")
							->setDescription("Include test suite and their respective test case information");
			$objPHPExcel->getActiveSheet()->setTitle('Webonise Lab');
			

			
			
			$objPHPExcel->setActiveSheetIndex(0)
			            ->setCellValue('A1', 'Test Suite Name')
			            ->setCellValue('B1', 'Test Case ID')
						->setCellValue('C1', 'Test Case Name')
						->setCellValue('D1', 'Summary')
						->setCellValue('E1', 'Preconditions')
						->setCellValue('F1', 'Execution Type')
						->setCellValue('G1', 'Steps')
						->setCellValue('H1', 'Expected Result');
			
			
			
			$size=count($testcase['name']);
			$suiteSize=count($testsuiteData);
			//$rowCounter=2;
			$col=2;
			for($j=0;$j<$suiteSize;$j++){
				...
				for($i=0;$i<$innerSize;$i++){
					...
					$objPHPExcel->setActiveSheetIndex(0)
			            ->setCellValue('B'.$col, $testCaseID)
						->setCellValue('C'.$col, $testcase[$j]['name'][$i])
						->setCellValue('D'.$col, $tcversions[$testCaseID]['summary'])
						->setCellValue('E'.$col, $tcversions[$testCaseID]['preconditions'])
						->setCellValue('F'.$col, $execution_type);
						//->setCellValue('G'.$col, $actions[$testCaseID])
						//->setCellValue('H'.$col, $expected_results[$testCaseID]);
						$stepsSize=count($steps[$testCaseID]['actions']);
						for($k=0;$k<$stepsSize;$k++,$col++){
							$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue('G'.$col, $steps[$testCaseID]['actions'][$k])
								->setCellValue('H'.$col, $steps[$testCaseID]['expected_results'][$k]);
						}
...
//Aameer Ausekar
function downloadTestPlanToXLS($planName,$content,$fileName){
	//echo 'in function';
	$objPHPExcel = new PHPExcel();
			$objPHPExcel->getProperties()->setCreator("Webonise lab")
							->setTitle("Excel Format For TestLink")
			...
			$objPHPExcel->getActiveSheet()->setTitle('Webonise Lab');
			$objPHPExcel->setActiveSheetIndex(0)
			            ->setCellValue('A1', 'Plane Name')
			            ->setCellValue('B1', 'Test Case')
			            ->setCellValue('A2', $planName);

			$size=count($content);
			for($i=0;$i<$size;$i++){
				$objPHPExcel->setActiveSheetIndex(0)
			            ->setCellValue('B'.($i+2), $content[$i]);
			}
			header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
			header("Content-Disposition: attachment; filename=\"testplan.xlsx\"");
			header("Cache-Control: max-age=0");
			$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
			$objWriter->save("php://output");
}
